...
describe("loadConfig", () => {
  ...
  it("reads exclude patterns", () => {
    setup({
      ".endpointtesterrc": JSON.stringify({ exclude: ["legacy/**", "test/**"] }),
    });
    const result = loadConfig(TEST_DIR);
  ...
  it("ignores unknown fields without throwing", () => {
    setup({
      ".endpointtesterrc": JSON.stringify({ framework: "express", unknownFutureField: true }),
    });
  ...
  it("throws when exclude is not an array of strings", () => {
    ...
    expect(() => loadConfig(TEST_DIR)).toThrow(/"exclude" must be an array of strings/);
  });
});
