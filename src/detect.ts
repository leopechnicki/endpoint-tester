  ...
  try {
    ...
    if (allDeps['@nestjs/core'] || allDeps['@nestjs/common']) {
      return {
        ...
        confidence: 'high',
        reason: '@nestjs/core found in package.json dependencies',
      };
    }
    ...
    if (allDeps['fastify']) {
      return {
        ...
        confidence: 'high',
        reason: 'fastify found in package.json dependencies',
      };
    }
    ...
    if (allDeps['koa']) {
      return {
        ...
        confidence: 'high',
        reason: 'koa found in package.json dependencies',
      };
    }
    ...
    if (allDeps['hono']) {
      return {
        ...
        confidence: 'high',
        reason: 'hono found in package.json dependencies',
      };
    }
    ...
    if (allDeps['express']) {
      return {
        ...
        confidence: 'high',
        reason: 'express found in package.json dependencies',
      };
    }
    ...
    if (allDeps['@types/express']) {
      return {
        framework: Framework.Express,
        confidence: 'high',
        reason: '@types/express found in package.json',
      };
    }
    ...
    if (allDeps['@types/koa']) {
      return {
        framework: Framework.Koa,
        confidence: 'high',
        reason: '@types/koa found in package.json',
      };
    }
    ...
    if (allDeps['@hono/node-server']) {
      return {
        framework: Framework.Hono,
        confidence: 'high',
        reason: '@hono/node-server found in package.json',
      };
    }
    ...
    try {
      ...
      if (content.includes('fastapi')) {
        return {
          ...
          confidence: 'high',
          reason: 'fastapi found in requirements.txt',
        };
      }
    ...
    try {
      ...
      if (content.includes('fastapi')) {
        return {
          framework: Framework.FastAPI,
          confidence: 'high',
          reason: 'fastapi found in pyproject.toml',
        };
      }
      if (content.includes('flask')) {
        return {
    ...
    try {
      ...
      if (content.includes('fastapi')) {
        return {
          framework: Framework.FastAPI,
          confidence: 'high',
          reason: 'fastapi found in setup.py',
        };
      }
      if (content.includes('flask')) {
        return {
          framework: Framework.Flask,
          confidence: 'high',
          reason: 'flask found in setup.py',
        };
      }
      if (content.includes('django')) {
        return {
          framework: Framework.Django,
          confidence: 'high',
          reason: 'django found in setup.py',
        };
      }
    } catch (err) {
    ...
    try {
      ...
      if (content.includes('spring-boot') || content.includes('spring-web')) {
        return {
          ...
          confidence: 'high',
          reason: 'Spring Boot found in pom.xml',
        };
      }
    } catch (err) {
      ...
      try {
        ...
        if (
          ...
          return {
            framework: Framework.Spring,
            confidence: 'high',
            reason: `Spring Boot found in ${gradleFile}`,
          };
        }
      } catch (err) {
...
function detectFromGoMod(directory: string): DetectionResult | null {
  ...
  try {
    ...
    return {
      ...
      confidence: 'medium',
      reason: 'go.mod present, no known router dependency',
    };
  } catch (err) {
...
async function detectFromFilePatterns(
  directory: string
): Promise<DetectionResult | null> {
  ...
  const tsFiles = await glob('**/*.{ts,js,mjs}', {
    cwd: directory,
    ...
    try {
      ...
      if (
        ...
        return {
          framework: Framework.NestJS,
          confidence: 'medium',
          reason: `NestJS decorators found in ${file}`,
        };
      }
      ...
      if (
        ...
        return {
          framework: Framework.Fastify,
          confidence: 'medium',
          reason: `Fastify import found in ${file}`,
        };
      }

      // Koa
      if (
        content.includes("require('koa')") ||
        content.includes('require("koa")') ||
        ...
        return {
          framework: Framework.Koa,
          confidence: 'medium',
          reason: `Koa import found in ${file}`,
        };
      }
      ...
      if (
        ...
        return {
          framework: Framework.Hono,
          confidence: 'medium',
          reason: `Hono import found in ${file}`,
        };
      }
      ...
      if (
        ...
        return {
          framework: Framework.Express,
          confidence: 'medium',
          reason: `Express import found in ${file}`,
        };
      }
    } catch (err) {
    ...
    try {
      ...
      if (
        ...
        return {
          framework: Framework.FastAPI,
          confidence: 'medium',
          reason: `FastAPI import found in ${file}`,
        };
      }
      if (content.includes('from flask') || content.includes('import flask')) {
        return {
          framework: Framework.Flask,
          confidence: 'medium',
          reason: `Flask import found in ${file}`,
        };
      }
      if (
        ...
        return {
          framework: Framework.Django,
          confidence: 'medium',
          reason: `Django import found in ${file}`,
        };
      }
    } catch (err) {
      warnOnReadError(file, err);
    ...
    try {
      ...
      if (content.includes('org.springframework')) {
        return {
          framework: Framework.Spring,
          confidence: 'medium',
          reason: `Spring import found in ${file}`,
        };
      }
    } catch (err) {
      warnOnReadError(file, err);
    ...
    try {
      ...
      if (content.includes('gin.Default()') || content.includes('gin.New()')) {
        return {
          framework: Framework.Gin,
          confidence: 'medium',
          reason: `Gin usage found in ${file}`,
        };
      }
      if (content.includes('echo.New()')) {
        return {
          framework: Framework.Echo,
          confidence: 'medium',
          reason: `Echo usage found in ${file}`,
        };
      }
      if (content.includes('chi.NewRouter()')) {
        return {
          framework: Framework.Chi,
          confidence: 'medium',
          reason: `Chi usage found in ${file}`,
        };
      }
      if (
        content.includes('http.HandleFunc') ||
        content.includes('mux.HandleFunc')
      ) {
        return {
          framework: Framework.NetHttp,
          confidence: 'medium',
          reason: `net/http usage found in ${file}`,
        };
      }
    } catch {
      /* ignore */
